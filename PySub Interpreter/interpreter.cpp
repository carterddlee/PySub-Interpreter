...
void Interpreter::run(LexicalAnalyzer::tokenLineType& line, expEvaluator& printEvaluator, bool& conditional, bool& skipElse, bool& inWhile)
{
	...
	for (auto i = line.begin(); i != line.end(); i++)
	{
		...
		if (i->second == LexicalAnalyzer::categoryType::IDENTIFIER && i + 1 != line.end() && (i + 1)->second == LexicalAnalyzer::categoryType::ASSIGNMENT_OP||
			inWhile && (i+1)->second == LexicalAnalyzer::categoryType::IDENTIFIER && i + 1 != line.end() && (i + 2)->second == LexicalAnalyzer::categoryType::ASSIGNMENT_OP)//There is assigning for an integer
		{
			if ((i + 2)->second == LexicalAnalyzer::categoryType::NUMERIC_LITERAL || (i + 2)->second == LexicalAnalyzer::categoryType::LEFT_PAREN ||
				(i + 2)->second == LexicalAnalyzer::categoryType::IDENTIFIER|| inWhile) //If the conditions fullfill, it is an equation
			{
				...
				if (inWhile)
					printEvaluator.AddtoSymbolTable((line.begin() + 1)->first, to_string(number));
				else
		...
		else if (i->second == LexicalAnalyzer::categoryType::KEYWORD && i->first == "print" || inWhile && (i+1)->first == "print") //If there is a print statement
		{
